...
const AiStudioView: React.FC<{
  ...
  setIsCartSelectorOpen: (isOpen: boolean) => void;
  ...
  const handlePhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        localStorage.setItem('userPhoto', base64String); // localStorage에 저장
  ...
  const handleAiGeneration = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        // 이 경우는 runGenAI 함수 내에서 이미 에러를 던졌을 가능성이 높습니다.
        // 하지만 만약을 위해 방어 코드를 추가합니다.
        throw new Error('AI가 이미지를 생성하지 못했습니다. 반환된 결과가 없습니다.');
      }
    } catch (error: any) {
          ...
          <div className="max-w-md mx-auto">
            {userPhoto ? (
              <div className="flex gap-3">
                <button onClick={() => setIsPhotoSheetOpen(true)} className="flex-1 bg-gray-100 text-gray-800 py-3.5 rounded-xl font-bold text-base active:scale-[0.98] transition-transform">사진 변경</button>
                <button 
                  onClick={() => {
                  ...
                  className="flex-1 bg-red-500 text-white py-3.5 rounded-xl font-bold text-base active:scale-[0.98] transition-transform"
                >
                  사진 삭제
                </button>
              </div>
            ) : (
              <button onClick={() => setIsPhotoSheetOpen(true)} className="w-full bg-black text-white py-3.5 rounded-xl font-bold text-base active:scale-[0.98] transition-transform">사진 등록</button>
            )}
          </div>
       </div>
