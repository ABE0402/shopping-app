...
export const OrderHistoryView: React.FC<OrderHistoryViewProps> = ({ orders, onBack, onRefresh }) => {
  // 디버깅: 주문 내역 확인
  React.useEffect(() => {
    console.log('📦 OrderHistoryView: orders prop:', orders);
    console.log('📦 OrderHistoryView: orders 개수:', orders.length);
    if (orders.length > 0) {
      console.log('📦 OrderHistoryView: 첫 번째 주문:', orders[0]);
    }
    
    // 페이지 진입 시 주문 내역 새로고침
    if (onRefresh) {
      console.log('🔄 OrderHistoryView: 주문 내역 새로고침');
      onRefresh();
    }
  }, [onRefresh]); // orders는 의존성에서 제외 (무한 루프 방지)
